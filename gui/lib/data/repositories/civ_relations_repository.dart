...
class CivRelation {
  ...
  /// Whether the GM has locked this relation (pipeline won't overwrite).
  final bool gmLock;
...
class CivRelationsRepository {
  ...
  /// Toggle gm_lock on a civ relation row.
  Future<void> setGmLock(int relationId, {required bool locked}) async {
    await _db.customStatement(
      'UPDATE civ_relations SET gm_lock = ? WHERE id = ?',
      [locked ? 1 : 0, relationId],
    );
  }
