...
- **bot/**: Python Discord bot + HTTP API + Claude agent. `python -m bot --db aurelm.db` starts the bot. **23 tools** = 22 read-only (listCivs, getCivState, getTurnDetail, searchLore, getEntityDetail, sanityCheck, timeline, compareCivs, searchTurnContent, getStructuredFacts, listSubjects, getSubjectDetail, getNotes, getFavorites, getCivRelations, deepExplore + 6 map tools: getMaps, getMapOverview, getCell, getCellHistory, getTerritory, findEntityOnMap) **+ `editMemory`** (the agent's own memory — create/update/forget; see `docs/agent-memory.md`). aiohttp HTTP server on :8473, discord.py for Discord gateway. **Agent backend: the etheryale proxy** (`ai.etheryale.com/v1`, OpenAI-compatible, fronting all Claude + GPT models) via the **OpenAI SDK** — NOT the Anthropic SDK, and no `claude -p` fallback (both removed). Auth is `x-api-key`. Per-request model picker + reasoning-effort knob (`reasoning_effort`) + visible-reasoning toggle (`include_reasoning`), all from the Flutter chat UI. **The system prompt is assembled PER REQUEST**: static SOUL.md + domain-knowledge, plus relevance-recalled agent notes and memories. 156 tests passing. Contract + traps: `docs/deployment.md`, the memory `reference_etheryale_proxy_llm_contract`.
