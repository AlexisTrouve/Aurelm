# Les Cercles de Vigile

*Technologie* -- Civilisation de la Confluence

| | |
|---|---|
| **Acquisition** | Tour 9 |
| **Categorie** | Autre |
| **Fiche narrative** | [Voir la page entite](../../entities/les-cercles-de-vigile.md) |

## Acquis en meme temps (Tour 9)

- [lance](lance.md)
- [cuillère](cuillere.md)
- [barreau de berceau](barreau-de-berceau.md)
- [artefacts](artefacts.md)
- [leg des ancêtres](leg-des-ancetres.md)
- [Porteurs de Flamme](porteurs-de-flamme.md)

## Arbre chronologique

**Tour 8** (precedent) : [outils des artisans](outils-des-artisans.md), [Tribunal de moeurs](tribunal-de-moeurs.md)
**Tour 9** (actuel) : **Les Cercles de Vigile**
**Tour 10** (suivant) : [argile vive](argile-vive.md), [roche braise](roche-braise.md), [lait de pierre](lait-de-pierre.md)

## Meme categorie : Autre

- [outils](outils.md) (Tour 1)
- [techniques](techniques.md) (Tour 1)
- [rites de déposition des morts](rites-de-deposition-des-morts.md) (Tour 3)
- [mémentos en mémoire des aïeux](mementos-en-memoire-des-aieux.md) (Tour 3)
- [la voix des cieux](la-voix-des-cieux.md) (Tour 4)
- [l'interprétation des présages dans les étoiles, le vent et les entrailles](l-interpretation-des-presages-dans-les-etoiles-le-vent-et-les-entrailles.md) (Tour 4)
- [pointes de flèches](pointes-de-fleches.md) (Tour 5)
- [roches striées](roches-striees.md) (Tour 5)
- [serpes grossières taillées dans des os](serpes-grossieres-taillees-dans-des-os.md) (Tour 6)
- [Les Larmes du Ciel](les-larmes-du-ciel.md) (Tour 6)
- [rhombes en pierre](rhombes-en-pierre.md) (Tour 7)
- [pipeau en bambou](pipeau-en-bambou.md) (Tour 7)
- [rhombes](rhombes.md) (Tour 7)
- [Tribunal de moeurs](tribunal-de-moeurs.md) (Tour 8)
- [cuillère](cuillere.md) (Tour 9)
- [barreau de berceau](barreau-de-berceau.md) (Tour 9)
- [artefacts](artefacts.md) (Tour 9)
- [leg des ancêtres](leg-des-ancetres.md) (Tour 9)
- [Porteurs de Flamme](porteurs-de-flamme.md) (Tour 9)
- [argile vive](argile-vive.md) (Tour 10)
- [roche braise](roche-braise.md) (Tour 10)
- [lait de pierre](lait-de-pierre.md) (Tour 10)
- [Regards-Libres](regards-libres.md) (Tour 11)
- [Lois du Sang](lois-du-sang.md) (Tour 11)
- [la Bête](la-bete.md) (Tour 11)
- [argile vivante](argile-vivante.md) (Tour 11)
- [rhombes pour les rituels et communications](rhombes-pour-les-rituels-et-communications.md) (Tour 12)
- [L'Appel de l'Abysse](l-appel-de-l-abysse.md) (Tour 12)
- [fresques](fresques.md) (Tour 13)
- [pigments](pigments.md) (Tour 13)
- [artefacts translucides](artefacts-translucides.md) (Tour 13)
- [techniques de creusage](techniques-de-creusage.md) (Tour 13)
- [Maîtrise des Profondeurs](maitrise-des-profondeurs.md) (Tour 13)
- [Grande Fresque](grande-fresque.md) (Tour 14)
- [Premiers Ancêtres](premiers-ancetres.md) (Tour 14)
- [Foyer du savoir](foyer-du-savoir.md) (Tour 14)
- [la Fresque des Âges](la-fresque-des-ages.md) (Tour 14)
- [chroniques de la Fresque](chroniques-de-la-fresque.md) (Tour 14)
- [Glyphes du Gouffre](glyphes-du-gouffre.md) (Tour 14)
- [Grande Prospection](grande-prospection.md) (Tour 14)

## Extrait narratif

> ...et le ciel se rencontrent. J'ordonne l'établissement de deux types de lieux sacrés qui formeront ensemble notre nouvelle extension vers les hauteurs. Les Cercles de Vigile seront établis sur des promontoires à mi-hauteur des montagnes, où l'œil peut embrasser toute la vallée. Ces lieux ne seront pas de simples postes d'...

---
[Retour a l'arbre technologique](../technologies.md)
